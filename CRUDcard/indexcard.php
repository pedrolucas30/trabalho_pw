...
<head>
    ...
    <title>Cardápio - Nosso Café</title>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 bg-white p-6 rounded-2xl shadow-md border border-amber-100">
            <div>
                ...
                <p class="text-amber-700/60 font-medium text-xs uppercase tracking-widest mt-1">Gerenciamento de produtos e estoque</p>
            </div>
            
            <a href="createcard.php" class="inline-flex items-center bg-amber-700 hover:bg-amber-800 text-white font-medium py-2.5 px-5 rounded-xl transition duration-200 shadow-md transform hover:-translate-y-0.5 active:translate-y-0 text-sm gap-2">
                ➕ Adicionar Item
            </a>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-amber-900 text-amber-50 uppercase text-xs tracking-wider font-semibold">
                            ...
                            <th class="py-4 px-6">Nome / Item</th>
                            <th class="py-4 px-6 w-36">Preço</th>
                            <th class="py-4 px-6 w-36 text-center">Quantidade</th>
                        ...
                        <?php if (empty($lista)): ?>
                            <tr>
                                <td colspan="5" class="py-8 text-center text-gray-400 bg-amber-50/10 italic text-sm">
                                    Nenhum item cadastrado no cardápio no momento.
                                </td>
                            </tr>
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-amber-50/40 transition duration-150 odd:bg-white even:bg-amber-50/10">
                                ...
                                <td class="py-4 px-6 text-center text-sm font-medium">
                                    <span class="inline-block px-3 py-1 rounded-full <?php echo $item['quantidade'] > 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100'; ?>">
                                        <?php echo $item['quantidade']; ?> un
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="updatecard.php?id=<?php echo $item['id']; ?>" 
                                           class="bg-amber-100 hover:bg-amber-200 text-amber-800 font-medium py-1.5 px-3 rounded-lg text-xs transition duration-150">
                                            Editar
                                        </a>
                                        <a href="indexcard.php?delete_id=<?php echo $item['id']; ?>" 
                                           onclick="return confirm('Deseja mesmo excluir este item do cardápio?')"
